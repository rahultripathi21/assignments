...
app.post('/todos',(req, res)=>{ 
  ...
  return fs.readFile('files/todo.json','utf-8', (error, data)=>{
      let todos = [];
      if(error) return res.status(400).send("File not avaialble");
      if(data){
      ...
      fs.writeFile('files/todo.json', jsonData,(err, data)=>{});
...
app.get('/todos/:id',(req, res)=>{
  ...
  return fs.readFile('files/todo.json', 'utf-8', (error, data)=>{
    if(error) return res.status(400).send("File not avaialble");
    if(!data) res.status(400).send('Not found');
    todos = JSON.parse(data);
